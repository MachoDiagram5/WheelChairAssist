...
void taskSetup() 
{
  configureLEDC();
    ...
    // Set PWM duty cycle for channel 2
    duty_cycle2 = 0;
    ledc_set_duty(LEDC_MODE, LEDC_CHANNEL2, duty_cycle2);
    ledc_update_duty(LEDC_MODE, LEDC_CHANNEL2);

    // Set PWM duty cycle for channel 3
    duty_cycle3 = 0;
    ledc_set_duty(LEDC_MODE, LEDC_CHANNEL3, duty_cycle3);
    ledc_update_duty(LEDC_MODE, LEDC_CHANNEL3);
...
void updateTask(void *blank)
{
    while(1) {
        ...
        if(gpio_get_level(BUTTON_FORWARD_GPIO) == 0) {//if foward button is pressed
            ...
            duty_cycle0 += 15;
            duty_cycle2 += 15;
            if(duty_cycle0 > LEDC_FULL_SPEED && duty_cycle2 > LEDC_FULL_SPEED) {
                duty_cycle0 -= 15;
                duty_cycle2 -= 15;
            }
         ...
         //pwm channel 1 & 3
            ...
            duty_cycle1 += 15;
            duty_cycle3 += 15;
            if(duty_cycle1 > LEDC_FULL_SPEED && duty_cycle3 > LEDC_FULL_SPEED) {
                duty_cycle1 -= 15;
                duty_cycle3 -= 15;
            }
        ...
        //only channel 1 decreses because of the constant high state nothing happens to channel 1
            ...
            duty_cycle2 -= 15; // Decrement duty_cycle1 for the channel2
            duty_cycle3 -= 15; // Decrement duty_cycle1 for the channel3
            if(duty_cycle0 < 0 && duty_cycle1 < 0) {
                duty_cycle0 += 15;
                duty_cycle1 -= 15;
            }
            if(duty_cycle2 < 0 && duty_cycle3 < 0) {
                duty_cycle2 += 15;
                duty_cycle3 -= 15;
            }
